# Claude Fable 5 — System Prompt (リファレンス標本)

> ⚠️ **これは外部リファレンス標本であり、Conclave 自身の成果物ではない。** Anthropic の Claude Fable 5 モデルのシステムプロンプトを、研究・学習目的で**原文のまま**収録したもの(ベンダーのコンテンツ)。
>
> - **なぜ Conclave に置くのか**: これは「本番 AI エージェントの統治文書」が実際どう書かれているかの一次資料。Conclave がプロジェクトごとに作る手助けをするもの(役割・憲法・トーン・検証規律)の、モデル本体版の実例である。Conclave 視点での読み解きと、各技法 → 6 本柱の対応は **[system-prompt-as-governance.md](system-prompt-as-governance.md)**。
> - **インストールされない**: `conclave init` はこのファイルをターゲットプロジェクトへコピーしない(統治キットの構成要素ではなく study material)。`references/` 全体が init 対象外。
> - **取り扱い**: 原文を改変せず収録。引用・再利用の際は、本プロンプト自身が説く copyright 規律(distillation 優先・15 語以上の逐語引用回避)に従うこと。
