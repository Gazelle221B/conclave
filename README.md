...
```bash
# このリポジトリから配布 tarball を作ってインストール (バージョンに依存しない書き方)
npm install -g "$(npm pack)"

conclave init <your-project>    # 統治キットを配置 + .conclave/manifest.json を記録
conclave check <your-project>   # 実在・非空・プレースホルダ残数・エイリアス・kit ドリフトを検査
```
...
| コマンド / オプション | 動作 |
|---|---|
| `conclave init [target]` | 統治キット一式を配置し、`.conclave/manifest.json` (kit バージョン + 全ファイルの SHA-256 + エイリアス方式) を記録 |
| `conclave check [target]` | 実在・非空の検査 / ドキュメント別プレースホルダ残数 / CLAUDE.md エイリアス検証 / kit ファイルのローカル改変 (ドリフト) 検出 |
| `conclave version` (`--version` / `-V`) | CLI バージョンを表示 |
| `--dry-run` | 書き込みなしで計画を表示。**実 run と同じ実現可能性チェックを通す**ため、dry-run が成功すれば実 run も成功する |
| `--force` | Conclave 管理ファイルを上書き (kit 更新時の再配置) |
| `--no-claude-alias` | CLAUDE.md エイリアスを作らない (既存の CLAUDE.md を保持) |
| `--json` | 機械可読出力 (`check`、または `init --dry-run`)。CI 連携用 |

`.conclave/manifest.json` は Git にコミットする。導入時の内容ハッシュが残るため、governance ドキュメントへの意図しないローカル改変 (ドリフト) を `conclave check` と `git diff` の両方で追える。

### CI で品質ゲート化する (GitHub Actions)

`check` を required check にすると、「governance ファイルの欠落・空化・エイリアス破壊」がマージをブロックする——憲法 P2 (人間専権ゲート) の機械化。
...
jobs:
  conclave:
    ...
    steps:
      ...
      # 導入時の kit と同じバージョンに固定する。未固定の npx は新リリースの
      # 要求ファイル追加で、リポジトリ側は無変更でも required check が赤になりうる
      - run: npx conclave-governance-kit@0.4.0 check . --json
```

FAIL 条件は「必須ファイルの欠落・空化・記録済みエイリアスの消失」。プレースホルダ残数と kit ドリフトは**警告**であり check を FAIL にしない (埋め忘れ検出を FAIL にしたい場合は `--json` の `placeholders` を自前のスクリプトで判定する)。
...
├── bin/
│   └── conclave.js            # ゼロ依存 CLI (init/check/version + install manifest + ドリフト検査)
├── test/
